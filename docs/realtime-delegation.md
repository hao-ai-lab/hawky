...
`session_task_control` supplies authoritative list/status/cancel/revise operations. Corrections create a replacement task and invalidate the old result. Task execution, result validity and response delivery are stored separately. Delivery describes the associated provider response, not proof that a person heard or understood every word.
