...
| MOSS-Transcribe-Diarize (0.9B) | Candidate **diarization + transcription front-end** for the multi-speaker/cross-modal follow-up — NOT an owner-verification model | End-to-end audio LLM (Qwen3-0.6B decoder + Whisper-Medium encoder, BF16, Apache-2.0) that jointly produces long-form transcription, timestamps, and **anonymous** diarization labels (`[S01]`/`[S02]`) plus acoustic events, CN/EN ([model card](https://huggingface.co/OpenMOSS-Team/MOSS-Transcribe-Diarize)). It solves a DIFFERENT problem from our voiceprint: it answers "who spoke when + what" within one clip but produces **no speaker embeddings and no cross-session identity** — S01 in clip A is unrelated to S01 in clip B, and there is no enrollment/verification of a known person. It is batch (not realtime) and GPU-heavy (single-H100 RTF ≈0.09), so it is not an on-device owner-recognition path. Complementary, not a replacement: a future multi-speaker pipeline could run MOSS for diarized segments + transcript, then attach our CAM++ embedding per segment and match against the encrypted owner template to turn anonymous `[S01]` into "the owner" vs "a stranger". This is the diarization front-end for PR 7 / the deferred multi-speaker (S1) work — it does not change the owner-first, realtime, on-device, privacy-first voiceprint architecture. |
