...
@Suite struct VoiceprintRealtimeVadTests {
    ...
    // MARK: - OFF-BY-DEFAULT timeline invariance (barge-in / timeline block is ungated)

    /// Off-by-default invariance for the ungated timeline block. `noteTimelineSpeechStarted`
    /// feeds `LiveTimelineStore` the REAL OpenAI item_id only, never the voiceprint-
    /// synthesized "ios_speech_N" speechWindowID. During the arg-less WebRTC warm-up the
    /// item_id is nil, so the timeline window's realtimeItemID must stay nil — exactly as
    /// it was before the VAD converter started surviving empty-JSON speech_started. This
    /// pins that the fix did not leak a synthesized id into the flag-OFF timeline state.
    @Test func warmupSpeechStartedLeavesTimelineRealtimeItemIDNil() {
        let timeline = LiveTimelineStore()
        // Mirrors noteTimelineSpeechStarted's warm-up call: itemID nil (empty-JSON WebRTC
        // VAD), and we must NOT pass the synthesized speechWindowID.
        let window = timeline.noteSpeechStarted(atNs: 1_000, realtimeItemID: nil)
        #expect(window.realtimeItemID == nil)
        _ = timeline.noteSpeechStopped(atNs: 2_000)
        #expect(timeline.turnSnapshots.last?.realtimeItemID == nil)
    }

    /// Even with the warm-up window carrying a nil realtimeItemID, a later transcript still
    /// attaches via the `transcript == nil` fallback (the synthesized id was never needed
    /// for correlation). Proves dropping the "ios_speech_N" fallback is behavior-preserving.
    @Test func warmupNilItemIDStillAttachesTranscriptViaFallback() {
        let timeline = LiveTimelineStore()
        _ = timeline.noteSpeechStarted(atNs: 1_000, realtimeItemID: nil)
        _ = timeline.noteSpeechStopped(atNs: 2_000)
        // A real OpenAI item_id never equals a synthesized "ios_speech_N", so attachment
        // must succeed through the transcript == nil fallback regardless.
        timeline.attachTranscript(itemID: "item_real_1", transcript: "hello there")
        #expect(timeline.turnSnapshots.last?.transcript == "hello there")
    }
