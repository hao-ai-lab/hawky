...
final class LiveSessionStore {
    ...
    /// set, so `liveConfig` would read the DRAFT config. Reading the effective
    /// config here keeps per-session overrides (e.g. a session that forces the
    /// camera off, or a distinct visual source/FPS) from being bypassed by the
    /// draft settings.
